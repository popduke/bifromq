...
public class RocksDBCPableKVSpaceRestoreBulkTest {
    private Path tmpRoot;
    private RocksDBCPableKVEngine engine;

    @BeforeMethod
    public void setup() throws Exception {
        tmpRoot = Files.createTempDirectory("kvspace_restore_bulk_");
    }
    ...
    @Test
    public void testBulkRestoreReplace() throws Exception {
        String spaceId = "space_bulk";
        ...
        int n = 5000;
        for (int i = 0; i < n; i++) {
            session.put(ByteString.copyFromUtf8("k" + i), ByteString.copyFromUtf8("v" + i));
        }
        session.done();
